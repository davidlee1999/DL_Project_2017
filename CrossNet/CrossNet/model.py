...
def training(loss_one, loss_two):
  ...
  optimizer_both = tf.train.AdamOptimizer(1e-5)
...
''' Some unfinished useless code
### Put common neurons at FRONT layer of the network
def build_net2(image_one, image_two, keep_prob_exclu_one=1, keep_prob_exclu_two=1,
               keep_prob_common_one=1, keep_prob_common_two=1):
  with tf.variable_scope("one_exclusive_1") as scope:
    matrix = _fc_matrix("matrix", [128, 784])
    bias = _fc_bias("bias", [128, 1])
    pre_activation = tf.add(tf.matmul(matrix, image_one), bias)
    fc_one_exclusive_1 = tf.nn.relu(pre_activation, name=scope.name)

  with tf.variable_scope("two_exclusive_1") as scope:
    matrix = _fc_matrix("matrix", [128, 784])
    bias = _fc_bias("bias", [128, 1])
    pre_activation = tf.add(tf.matmul(matrix, image_two), bias)
    fc_two_exclusive_1 = tf.nn.relu(pre_activation, name=scope.name)

  with tf.variable_scope("one_common_1") as scope:
    matrix = _fc_matrix("matrix", [256, 784])
    bias = _fc_bias("bias", [256, 1])
    pre_activation = tf.add(tf.matmul(matrix, image_one), bias)
    fc_one_common_1 = tf.nn.relu(pre_activation, name=scope.name)

  with tf.variable_scope("two_common_1") as scope:
    matrix = _fc_matrix("matrix", [256, 784])
    bias = _fc_bias("bias", [256, 1])
    pre_activation = tf.add(tf.matmul(matrix, image_two), bias)
    fc_two_common_1 = tf.nn.relu(pre_activation, name=scope.name)

  with tf.variable_scope("one_exclusive_2") as scope:
    matrix = _fc_matrix("matrix", [64, 128])
    bias = _fc_bias("bias", [64, 1])
    pre_activation = tf.add(tf.matmul(matrix, fc_one_exclusive_1), bias)
    fc_one_exclusive_2 = tf.nn.relu(pre_activation, name=scope.name)

  with tf.variable_scope("two_exclusive_2") as scope:
    matrix = _fc_matrix("matrix", [64, 128])
    bias = _fc_bias("bias", [64, 1])
    pre_activation = tf.add(tf.matmul(matrix, fc_two_exclusive_1), bias)
    fc_two_exclusive_2 = tf.nn.relu(pre_activation, name=scope.name)

  with tf.variable_scope("common_2") as scope:
    matrix = _fc_matrix("matrix", [128, 256])
    bias = _fc_bias("bias", [128, 1])
    pre_activation_one = tf.add(tf.matmul(matrix, fc_one_common_1), bias)
    pre_activation_two = tf.add(tf.matmul(matrix, fc_two_common_1), bias)
    fc_common_one_2 = tf.nn.relu(pre_activation_one, name=scope.name)
    fc_common_two_2 = tf.nn.relu(pre_activation_two, name=scope.name)
